        ...
        <div className="space-y-4">
            {
                tags.length > 0 ? (
                    tags.map((t) => (
                        <button key={t._id.toString()} onClick={() => { }} className="px-3 py-1 rounded-full bg-white/90 dark:bg-neutral-800/80 shadow-sm">#{t.tag}</button>
                    ))
                ) : (
